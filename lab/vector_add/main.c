...
int main(int argc, char *argv[])
{
    ...
    // host_a.data and host_a.shape[0-1]
    ...
    // Matrix Answer (Output.raw file)
    const char *input_file_c = argv[3];
    err = LoadMatrix(input_file_c, &answer);
    CHECK_ERR(err, "LoadMatrix");

    // Matrix D (Output File)
    ...
    clEnqueueReadBuffer(queue, device_c, CL_TRUE, 0, sizeof(float) * host_a.shape[0] * host_a.shape[1], host_c.data, 0, NULL, NULL);
